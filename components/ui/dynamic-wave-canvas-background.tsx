...
const HeroWave = () => {
  ...
  useEffect(() => {
    ...
    const fastSin = (x: number) => {
      ...
      return SIN_TABLE[index >= 0 ? index : index + 1024];
    };

    const fastCos = (x: number) => {
      const index = Math.floor(((x % (Math.PI * 2)) / (Math.PI * 2)) * 1024) & 1023;
      return COS_TABLE[index >= 0 ? index : index + 1024];
    };
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none -z-10 object-cover" />;
};
